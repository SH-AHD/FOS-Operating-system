...
void* create_user_kern_stack(uint32* ptr_user_page_directory)
{
    ...
    int res=get_page_table(ptr_user_page_directory, ((uint32)(ptr)), &ptr_page_table);
    ptr_page_table[PTX(ptr)]=ptr_page_table[PTX(ptr)]&(~PERM_PRESENT);
    return ptr;
...
void initialize_uheap_dynamic_allocator(struct Env* e, uint32 daStart, uint32 daLimit)
{
	...
	//panic("initialize_uheap_dynamic_allocator() is not implemented yet...!!");
	 e->engy_start = (uint32*)daStart;
	    e->engy_hard_limit = (uint32*)daLimit;
	    e->engy_sbrk = (uint32*)daStart;
