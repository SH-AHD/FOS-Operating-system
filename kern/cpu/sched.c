...
//======================================
void sched_init_PRIRR(uint8 numOfPriorities, uint8 quantum, uint32 starvThresh)   // LINE 247
{
...
//	panic("Not implemented yet");
	...
	quantums[0]=quantum; // do i need to loop and set the first element.
	kclock_set_quantum(quantums[0]);
	...
	for(int i=0;i<numOfPriorities;i++){
		//ProcessQueues.env_ready_queues[i]=(struct Env_Queue)kmalloc(sizeof(struct Env_Queue));
...
{
    /* To protect process queues in multi-CPU environments */
    if (!holding_spinlock(&ProcessQueues.qlock))
    ...
    struct Env *next_env = NULL;
    // step 1 ////// if there’s a current process on the CPU,then place it back in the ready queue
    if (current_env != NULL) {
        // i will ensure the process is set to ready state
        current_env->env_status = ENV_READY;
        // then i will insert the process back into the corresponding ready queue based on its priority
        enqueue(&ProcessQueues.env_ready_queues[current_env->priority], current_env);
    }
    // step 2 ////////////i will select the next environment to run on the CPU
    ...
    // step 3 /////////////i will set the CPU quantum for the selected process
    if (next_env != NULL) {
        set_cpu_proc(next_env);
        next_env->env_status = ENV_RUNNING;
        kclock_set_quantum(quantums[0]);
    }
    //step 4 //////////i will return the next environment
    return next_env;
}
...
void clock_interrupt_handler(struct Trapframe* tf)
{
	if (isSchedMethodPRIRR())
	{
	    ...
	    if (env->waiting_ticks > shahd_StarvThresh) {
	  ////////promote the process to a higher priority queue
	   int new_priority = (env->priority > PRI_MIN) ? env->priority - 1 : PRI_MIN;
         if (new_priority != env->priority) {
         // i will remove from the current queue
           LIST_REMOVE(&ProcessQueues.env_ready_queues[i], env);
              // i will update the priority and reset waiting ticks
                  env->priority = new_priority;
                  env->waiting_ticks = 0;
                  ////////insert into the higher-priority queue
	   enqueue(&ProcessQueues.env_ready_queues[new_priority], env);
 }
 }
	else {
 //finally, i will increment the waiting ticks for processes that haven't exceeded the threshold
		   env->waiting_ticks++;
	 }
	 }
		        }
