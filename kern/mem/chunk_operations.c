...
void* sys_sbrk(int numOfPages)
{
    ...
    uint32 totalsize_sys_sbrk=numOfPages*PAGE_SIZE;
    uint32* Ubrk=env->engy_sbrk;
    uint32* prev_Ubrk=env->engy_sbrk;
    uint32* new_Ubrk=(uint32*)(((uint32)Ubrk)+totalsize_sys_sbrk);
    if(totalsize_sys_sbrk==0){
    	//cprintf("  Ubrk  %p \n",Ubrk);
    	return (void*)Ubrk;
    }
    if(totalsize_sys_sbrk>0 && new_Ubrk<=env->engy_hard_limit){
    	//uint32* end_blk=(uint32*)((uint32)env->engy_sbrk-sizeof(int));
    	env->engy_sbrk=(uint32*)((uint32)env->engy_sbrk+totalsize_sys_sbrk);
    	allocate_user_mem(env,(uint32)Ubrk,totalsize_sys_sbrk);
    	//cprintf("  Ubrk  %p \n",Ubrk);
    	return (void*)Ubrk;

    }
    if(new_Ubrk>env->engy_hard_limit || Ubrk==env->engy_hard_limit){
    		return (void*)-1 ;
    }
    return NULL;
}
...
void allocate_user_mem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	//panic("allocate_user_mem() is not implemented yet...!!");
	 // Initialize the current page address to the starting virtual address
...
//cprintf("numOfPages %d  \n",numOfPages);
	...
	for(int i = 0; i < numOfPages; i++){
		result = get_page_table(e->env_page_directory, pagePtr, &ptr_page_table);
	//	cprintf("result %d  \n",result);

		if(result == TABLE_NOT_EXIST) //if table not exist i will create it
			ptr_page_table = create_page_table(e->env_page_directory, pagePtr);
		pt_set_page_permissions(e->env_page_directory, pagePtr, PERM_MARKED, 0);
		pagePtr += PAGE_SIZE;
	}
	//cprintf("SMALLOC user mem done %d \n ");
}
...
void free_user_mem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	//uint32 pagenum=(virtual_address-USER_HEAP_START)/PAGE_SIZE;//set perm bits=0 => get page #
	//cprintf( "pagenum %d \n",pagenum);
	uint32 pagePtr = virtual_address;
    ...
    uint32* ptr_page_table=NULL;
	 ...
	 //if table not exist i will return

	 for(uint32 i=0;i<numOfPages;i++){
		 int result = get_page_table(e->env_page_directory, pagePtr, &ptr_page_table);
		 	//cprintf( "result  %d \n",result);
		 	if(result == TABLE_NOT_EXIST) {return;}
		//  loop on the given range to free pages & remove pages from page file & ws
	 // use get perm func to know if marked (get perm& marked perm)
	 uint32 pageperm=pt_get_page_permissions(e->env_page_directory, pagePtr);
	 //cprintf( "pageperm %d \n",pageperm);
	 	 pt_set_page_permissions(e->env_page_directory, pagePtr, 0 , PERM_MARKED); // make page unmarked
	...
	// cprintf( "env_page_ws_invalidate( e, virtual_address); %d \n");
	...
	// cprintf( "virtual_address  %d \n",pagePtr);
	// cprintf("KFREE FREE USER MEM DONE!");
	//cprintf("i for free %d\n",i);
	 }
